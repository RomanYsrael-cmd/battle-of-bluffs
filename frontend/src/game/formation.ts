import { coordinateKey, isPlayerOneFormationPosition, type Position } from './coordinates'
...
export const isValidFormation = (inventory: LocalPiece[], placements: Placements): boolean => {
  ...
  return inventory.length === FORMATION_PIECE_COUNT
    ...
    && positions.every(isPlayerOneFormationPosition)
...
export const placeOrSwap = (
  ...
  if (!isPlayerOneFormationPosition(destination)) return placements
