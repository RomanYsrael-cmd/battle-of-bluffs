...
for (const viewport of [
  ...
  test(`active workspace scales to fit ${viewport.width}x${viewport.height} without scrolling`, async ({ page }) => {
    ...
    expect(board!.width).toBeCloseTo(Math.min(928, viewport.width * .55, (viewport.height - 160) * 1.125), 0)
    expect(await page.locator('.match-app-shell').evaluate((node) => getComputedStyle(node).position)).toBe('fixed')
...
test('terminal status and complete board scale to fit 1366x768 without scrolling', async ({ page }) => {
  ...
  expect(board!.width).toBeGreaterThanOrEqual(675)
...
test('formation board, 21-piece tray, actions and dock fit 1366x768', async ({ page }) => {
  ...
  expect(actions!.x + actions!.width).toBeLessThanOrEqual(connection!.x)
  expect(actions!.y).toBeLessThan(deadline!.y + deadline!.height)
  expect(actions!.y + actions!.height).toBeGreaterThan(deadline!.y)
